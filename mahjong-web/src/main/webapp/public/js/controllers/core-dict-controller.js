...
function CoreDictionaryCtrl($scope, dictionaryService, partOfSpeechRepository, conceptRepository, $routeParams) {

    $scope.alphabetical = !!$routeParams.index ? $routeParams.index.toUpperCase() : "A";
    $scope.wordIndexes = ["A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "O", "P", "Q", "R", "S", "T", "U", "W", "X", "Y", "Z"];
    $scope.pinyinIndexes = getRelatedPinyinList($scope.alphabetical);
    $scope.wordHeads = [];
    $scope.wordItems = [];
    $scope.selectedIndex = $scope.alphabetical;
    $scope.selectedPinyin = $routeParams.pinyin || $scope.pinyinIndexes[0];
    $scope.selectedHead = $routeParams.wordHead || null;
    ...
    $scope.switchPinyin = function (pinyin) {
        dictionaryService.getWordHeads('core', pinyin, function (result) {
            ...
            var exists = _.any(result, function (word) {
                return $scope.selectedHead == word;
            });
            if (exists) {
                getWords($scope.selectedHead);
            } else {
                getWords(result[0]);
            }
        });
...
CoreDictionaryCtrl.$inject = ['$scope', 'DictionaryService', 'PartOfSpeechRepository', 'ConceptRepository', '$routeParams'];
