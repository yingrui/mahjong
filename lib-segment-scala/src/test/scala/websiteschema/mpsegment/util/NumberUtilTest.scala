...
class NumberUtilTest {
  ...
  @Test
  def should_know_how_to_convert_Chinese_number_to_English() {
  ...
  @Test
  def should_know_how_to_convert_English_number_to_Chinese() {
    Assert.assertEquals(NumberUtil.toChineseNumber("111111123,456.12"), "一千一百一十一亿一千一百一十二万三千四百五十六点一二")
    Assert.assertEquals("负一百零三亿零九百三十万零九百三十二", NumberUtil.toChineseNumber(-10309300932L))
  }
}
