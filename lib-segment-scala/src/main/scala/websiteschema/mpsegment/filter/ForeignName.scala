...
class ForeignName {
  ...
  def loadNameWord() {
    for (i <- 0 until foreignNameChars.length) {
      foreignNameCharsHashMap += (foreignNameChars.charAt(i) -> 1)
    }

  }

  private def containsChar(s: Char): Boolean = {
    return foreignNameCharsHashMap.contains(s)
  }

  def isForiegnName(s: String): Boolean = s.forall(ch => containsChar(ch))
  ...
  private val foreignNameCharsHashMap = mutable.HashMap[Char, Int]()
}
