...
class BagOfWordNetworkTest extends FunSuite with Matchers {
  ...
  test("should calculate layer1 grads for given input and output") {
    ...
    val grad = network.computeLayer1Grads(input, output)
    grad(0)._2 should be ((1D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha)
    grad(1)._2 should be ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha)
    grad(2)._2 should be ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha)
  }
  ...
  test("should update layer1 weights and compute back propagation errors") {
    ...
    val input = Array[Double](1D, 1D)
    val output = Array[(Int, Int)]((0, 1), (1, 0), (2, 0))
    val grads = network.computeLayer1Grads(input, output)
    ...
    layer1Weight(0)(0) should be (1D + ((1D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))
    layer1Weight(0)(1) should be (1D + ((1D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))

    layer1Weight(1)(0) should be (1D + ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))
    layer1Weight(1)(1) should be (1D + ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))

    layer1Weight(2)(0) should be (1D + ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))
    layer1Weight(2)(1) should be (1D + ((0D - Word2VecUtil.simplifiedSigmoid(2D)) * network.alpha))
  }
