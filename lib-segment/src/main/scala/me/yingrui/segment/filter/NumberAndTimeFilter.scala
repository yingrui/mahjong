...
import me.yingrui.segment.util.NumberUtil
...
class NumberAndTimeFilter extends AbstractSegmentFilter {
            ...
            if (isNotMarked(index)) {
                if (segmentResult.getPOS(index) == posUN) {
                    if (segmentResult.getWord(index).equals("-") || segmentResult.getWord(index).equals("－")) {
                        ...
                        val l1 = isNumerical(segmentResult.getWord(index))
                        if (l1 == 1) {
                            segmentResult.setPOS(index, posM)
                        }
                    }
    ...
    private def recognizeChineseNumber(length: Int) {
        for (wordI <- 0 until length) {
            if (segmentResult.getPOS(wordI) == posUN) {
                val j1 = isNumerical(segmentResult.getWord(wordI))
                if (j1 == 1) {
    ...
    private def isNumberSeparator(wordIndex: Int) : Boolean = {
        var flag = false
        val length = segmentResult.length()
        val wordStr = segmentResult.getWord(wordIndex)
        if (wordStr.length() == 1) {
    ...
    private val posM = POSUtil.POS_M
    private val posUN = POSUtil.POS_UNKOWN
    private val posT = POSUtil.POS_T
    private val posNR = POSUtil.POS_NR
    private val wuLiuWan = "伍陆万"
